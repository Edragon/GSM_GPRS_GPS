...
{
	u8 *p1; 
	UART1_SendString("\r\-Elec SIM900A 测试程序\r\n");  
	CLR_Buf2(); 
...
u8 sim900a_gsminfo()
{
	...
	if(sim900a_send_cmd("AT+CCLK?","OK",3)==0)		//查询时间
	{ 
		p1=(u8*)strstr((const char*)(Uart2_Buf),"\"");
	...
	while(1)
	{
		...
		switch(mode)
		{
          ...
          delay_ms(10);
					...
					UART1_SendData(*(p1+2));//SIM900A和SIM808的差异，SSIM808空了一个空位
					UART1_SendLR();	
					CLR_Buf2();
				}
				break;
...
u8 sim900a_sms_test()
{
  ...
  sim900a_send_cmd("AT+CNMI=3,1,0,0,0","OK",3);		//设置新短信只上报位置
	...
	UART1_SendString("发送短信：请按要求输入电话+内容(非中文)，以回车结束:如137xxxx+-electron\r\n");
