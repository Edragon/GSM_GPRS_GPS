...
int main(void)
{
	...
	sim800c_send_cmd("AT+DDET=1,0,0","OK",200); //开启DTMF  
	sim800c_send_cmd("AT+CTTSPARAM=50,0,50,55,1","OK",200); //TTS设置
                          // 音量 模式 音调 速度 通道
	u2_printf("ATD%s;\r\n",callbuf);//拨号	
  ...
  while(1)
  {
		if(USART2_RX_STA&0X8000)		//接收到数据
		{
			if(sim800c_check_cmd("+CLIP:"))//接收到来电
			{
				...
				delay_ms(1000);
				sim800c_send_cmd("AT+CTTS=2,\"你好，打开LED请按1，关闭LED请按2\"","OK",200);
			}
			...
			else if(sim800c_check_cmd("+DTMF:"))
			{
				p1=sim800c_check_cmd("+DTMF:");   //+DTMF: 1       
				if(p1[7]=='1')
				{
					sim800c_send_cmd("AT+CTTS=0","OK",200);  //停止播放
					sim800c_send_cmd("AT+CTTS=2,\"LED已打开\"","OK",200);
					LED1=0;
				}
				else if(p1[7]=='2')
				{
					sim800c_send_cmd("AT+CTTS=0","OK",200);  
					sim800c_send_cmd("AT+CTTS=2,\"LED已关闭\"","OK",200);
					LED1=1;  
				}
				else
				{
					sim800c_send_cmd("AT+CTTS=0","OK",200);  
					sim800c_send_cmd("AT+CTTS=2,\"无效按键\"","OK",200); 
				}
			}
