...
#elif defined(__SIMCOM_PROJ_SIM800F__)  || defined(__SIMCOM_PROJ_SIM800A__)
//SIM800F
typedef enum {
    EAT_PIN3_DTR        = 3,  /* GPIO, EINT */
...
*     EAT_FALSE The operation false.
* NOTE:
*    The pin only for defined in EatPinName_enum can used in EINT.
*    for example:
*        EAT_PIN28_GPIO2 = 28, // GPIO, EINT.
*****************************************************************************/
...
* Parameters :
*     clk  EatSpiClk_enum [IN] The freg of the SPI_CLK, please see EatSpiClk_enum.
*     wire EatSpiWire_enum [IN] Connect mode, 4 wire will use LSA0 pin as D/C(Date/Command), 3 wire not use, please see EatSpiWire_enum.
*     bit  EatSpiBit_enum [IN] The transmit speed, please see EatSpiBit_enum.
*     enable_SDI eat_bool [IN] Whether use MISO.
*     enable_cs eat_bool [IN] Whether use SPI_CS control chip selection when read or write SPI, if not, can control by APP write specific GPIO.
* Returns:
*     EAT_TRUE  The operation Sucess.
*     EAT_FALSE The operation false.
* NOTE:
*   Only SIM800W platform, the SPI_CLK can be configured. In other platform the SPI_CLK is fixed, is about 700kHz.
*****************************************************************************/
...
*     is_command eat_bool [IN] The type want to write, EAT_TRUE-command, EAT_FALSE-data.
* Returns:
*     EAT_TRUE  The operation Sucess.
*     EAT_FALSE The operation false.
* NOTE:
...
* Description: Reads the ADC value, the unit of period is 2.3 millisecond.
* Parameters:
*     pin  EatPinName_enum [IN] The pin want to read ADC. Note is only in
*                          EAT_PINx_ADCx pin.
*     period  unsigned int [IN] The mode want to read .
*                          0: read once.
*                          other: Periodically read, the unit is 2.3ms. 
*                          such as : period=1000 read the adc one time in 2.3 second.
...
*     EAT_FALSE The operation false.
* NOTE:
*     The unit of parameter "period_tick" is 2.3ms.
*****************************************************************************/
extern eat_bool (* const eat_adc_get)(EatPinName_enum pin, unsigned int period_tick, eat_adc_callback_func callback);

/*****************************************************************************
* Function :  eat_adc_get_ms
* Description: Reads the ADC value, the unit of period is millisecond.
* Parameters:
*     pin  EatPinName_enum [IN] The pin want to read ADC. Note is only in
*                          EAT_PINx_ADCx pin.
*     period_ms  unsigned int [IN] The mode want to read .
*                          0: read once.
*                          other: Periodically read, the unit is millisecond. 
...
*     EAT_FALSE The operation false.
* NOTE:
*     The unit of parameter "period_ms" is millisecond.
*****************************************************************************/
extern eat_bool (* const eat_adc_get_ms)(EatPinName_enum pin, unsigned int period_ms, eat_adc_callback_func callback);
...
* Description: Exports the PWM in PWM PIN.
* Parameters:
...
*     No returen value (void)
* NOTE:
*    The pin only for defined in EatPinName_enum can used in PWM.
*    For example:
*    EAT_PIN16_NETLIGHT = 16,  // GPIO, PWM 
*****************************************************************************/
...
*     No returen value (void)
* NOTE:
*     The module will shutdown, if enable this function and long press powerkey over 1.2 sencond. 
*****************************************************************************/
...
*     The max length of data(reading and writing) is 8 bytes once.
*****************************************************************************/
...
*      Failed:     Please refer to EAT_DEV_STATUS_T.
* NOTE:
*     The max length of data is 8 bytes once.
*****************************************************************************/
...
* Description: Get module temperature asynchronous.
* Parameters:
